...
def beam_search(model, src, src_mask, max_len, pad, bos, eos, beam_size, device):
    ...
    def beam_decode_step(
        ...
        def predict_word(dec_seq, enc_output, n_active_inst, n_bm):
            ...
            dec_embedding = model.decoder_embedding(dec_seq)
            out = model.decoder(enc_output, src_mask,
                                dec_embedding,
    ...
    with torch.no_grad():
        bs, len, c, w, h = src.size()

        src = src.view(bs * len, c, w, h)
        embedding = model.encoder_embedding(src)
        src_enc = model.encoder(embedding, src_mask)
...
def batch_greedy_decode(model, src, src_mask, max_len=64, start_symbol=2, end_symbol=3):
    batch_size, len, c, w, h = src.size()

    results = [[] for _ in range(batch_size)]
    stop_flag = [False for _ in range(batch_size)]
    count = 0

    src = src.view(batch_size * len, c, w, h)
    embedding = model.encoder_embedding(src)
    memory = model.encoder(embedding, src_mask)
    ...
    for s in range(max_len):
        ...
        dec_seq = model.decoder_embedding(Variable(tgt))
        out = model.decoder(memory, src_mask,
                            dec_seq,
                            Variable(tgt_mask))
